...
sub get_node {
    ...
    if ($@ || (! ref($content))) {
        ...
        print STDERR "[error] unable to GET node $node from Shock: ".$content->{error}."\n";
        return undef;
    } else {
        return $content->{data};
    }
...
sub download_to_path {
    ...
    eval {
        my $get = undef;
        if ($self->token) {
            $get = $self->agent->get($self->shock_url.'/node/'.$node.'?download', 'Authorization' => "OAuth ".$self->token);
        } else {
    ...
    if ($@ || (! $content)) {
        ...
        print STDERR "[error] unable to GET file $node from Shock: ".$content->{error}."\n";
        return undef;
    } else {
        if (open(FILE, ">$path")) {
            print FILE $content;
            ...
            print STDERR "[error] unable to write to $path\n";
            return undef;
        }
    }
}
...
sub upload {
    ...
    if ($data && $attr) {
        $content = [upload => $self->_get_handle($data), attributes => $self->_get_handle($attr)];
    } elsif ($data) {
    ...
    eval {
        ...
        if ($method eq 'POST') {
            if ($self->token) {
                $res = $self->agent->post($url, $content, Content_Type => 'form-data', Authorization => "OAuth ".$self->token);
            } else {
                $res = $self->agent->post($url, $content, Content_Type => 'form-data');
            }
        } else {
            if ($self->token) {
                $res = $self->agent->put($url, $content, Content_Type => 'form-data', Authorization => "OAuth ".$self->token);
            } else {
                $res = $self->agent->put($url, $content, Content_Type => 'form-data');
            }
        }
    ...
    if ($@ || (! ref($response))) {
        ...
        print STDERR "[error] unable to $method data to Shock: ".$response->{error}."\n";
    } else {
