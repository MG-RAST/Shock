		...
		if index {
			if query["index"][0] == "size" {				
				if part {
					...
					if chunksize {
						csize, err = strconv.ParseInt(query["chunksize"][0], 10, 64); if err != nil {
					...
					r, err := os.Open(node.DataPath()); if err != nil {
					...
					for _, p := range query["part"] {
						pInt, err := strconv.ParseInt(p, 10, 64); if err != nil {
							cx.RespondWithErrorMessage("Invalid index part", http.StatusBadRequest)
							return
						}
						pos, length, err := node.File.SizeOffset(pInt, csize); if err != nil {
							cx.RespondWithErrorMessage("Invalid index part", http.StatusBadRequest)
							return
						}
						size += length
