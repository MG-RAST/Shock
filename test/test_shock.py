...
# and passed as the files= parameter to requests.put
    ...
    if DEBUG: print("POST", TESTURL, TESTDATA, TESTHEADERS)
    response = requests.post(TESTURL, headers=TESTHEADERS, files=FILES)
    data = json.loads(response.content.decode("utf-8"))
    assert data["status"] == 200
    NODES += [data["data"]["id"]]
    if DEBUG: print("PUT", SHOCK_URL+ "/node/" + NODES[-1], TESTDATA)
    r = requests.put(SHOCK_URL + "/node/" + NODES[-1], files=TESTDATA, headers=TESTHEADERS)
    FILES = {'upload': open(DATADIR + 'BBB.txt', 'rb')}
    if DEBUG: print("POST", TESTURL, FILES)
    response = requests.post(TESTURL, headers=TESTHEADERS, files=FILES)
    data = json.loads(response.content.decode("utf-8"))
    NODES += [data["data"]["id"]]
    if DEBUG: print("PUT", SHOCK_URL+ "/node/" + NODES[-1], TESTDATA)
...
def confirm_nodes_project(NODES, PROJECT):
     for NODEID in NODES:
        ...
        TESTHEADERS = {"Authorization": "OAuth {}".format(TOKEN)}
        if DEBUG: print("curl '{}' -H 'Authorization: Oauth {}'".format(TESTURL, TOKEN))
...
def test_download_zip_GET():
    ...
    TESTHEADERS = {"Authorization": "OAuth {}".format(TOKEN)}
    if DEBUG: print("GET", TESTURL, TESTDATA)
    response = requests.get(TESTURL, headers=TESTHEADERS, params=TESTDATA)
#    if DEBUG: print(response.content)
    ...
    if DEBUG: print("curl '{}' -H 'Authorization: Oauth {}' -G -d {}".format(TESTURL, TOKEN, TESTDATA))
    response = requests.get(TESTURL, headers=TESTHEADERS, params=TESTDATA)
    data = json.loads(response.content.decode("utf-8"))
...
def test_download_tar_POST():
    ...
    if DEBUG: print("POST", TESTURL, TESTDATA)
